...
| `scripts/check-branding.sh` | Gate: upstream brand names allowed only in LICENSE + canonical attribution |
...
- **No upstream brand names** (the project this derives from) anywhere
  except `LICENSE` and the one canonical attribution sentence in
  README/CHANGELOG. `./scripts/check-branding.sh` enforces this — run it
  after editing docs.
